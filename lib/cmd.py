...
from lib import params, utils, sounds
...
def on_show_devices(cmd: pcmd.Command, args: List[str], json: bool):
    ...
    inputs = ['{:02d}: {}'.format(*inf) for inf in fil_d('maxInputChannels')]
    outputs = ['{:02d}: {}'.format(*inf) for inf in fil_d('maxOutputChannels')]
    if not json:
        # print('Devices:\n ', end='')
        # print('\r Input:\n   ', end='')
